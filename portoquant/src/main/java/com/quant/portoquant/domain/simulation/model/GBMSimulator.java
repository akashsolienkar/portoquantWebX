package com.quant.portoquant.domain.simulation.model;
...
public class GBMSimulator implements SimulationRunner {
    ...
    public double generate(SimulationContext context) {
        ...
        for (int i = 1; i <= ctx.getSteps(); i++) {
            ...
            double mu = ctx.getExpectedReturnModel().getExpectedReturn(i);
            double sigma = ctx.getVolatilityModel().getVolatility(i);
